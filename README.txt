...
Batchelder's. With this replacement it is also possible to write XML output.
...
In your buildout, add a part like this::
    ...
    eggs = 
        my.package
    defaults = ['-x', '--exit-with-status', '--auto-color', '--auto-progress', '--coverage-xml', '--coverage-branch']
...
test reports will be written to ``parts/test/reports`` directory::
    ...
    $ bin/test -s my.package --coverage-module my.package
